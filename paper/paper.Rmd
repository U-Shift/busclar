...
Finally, network density can be analyzed at the route level, with `get_route_frequency_hourly()`.
This analysis results in a dataset with the frequency of each route (`shape_id`) at each hour of the day.
For an aggregated geospatial analysis, this function accepts an `overline` flag, which overlaps routes in street segments, creating a single route network with sum frequencies.
This aggregation is performed using `stplanr::overline2()` [@stplanr].

To compare the streets with higher transit density and the existing bus lanes, `osm_bus_lanes()` allows to export the roads classified as bus lanes in *OpenStreetMap*, for a given bounding box.
...
```{r MapResults, fig.cap="Road segments with higher bus frequency, in Lisbon, at 8 am. The larger and yellowish lines represent the streets with higher hourly bus frequency. Carto basemap.", fig.link="http://www.rosafelix.bike/transit/carris_8h.html"}
# include figure
