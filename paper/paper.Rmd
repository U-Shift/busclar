...
-   Existing traffic conditions, such as the number of vehicles per lane;
...
This paper focuses in the identification of the bus frequency per hour for each segment of the road network, using General Transit Feed Specification (GTFS) data -- which is widely available for many transit agencies.
Identifying the corridors with highest bus frequency is critical to prioritize new bus lanes and support its implementation.

For this analysis, we present [`GTFShift`](https://u-shift.github.io/GTFShift/), an open-source R package to identify priorities for bus lanes' implementation [@gshift2025].
It extends the capabilities of other R libraries that deal with this transit standard, such as `tidytransit` [@tidytransit] and `gtfstools` [@gtfstools]. 
