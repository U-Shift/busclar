...
```
```{r tabresults, echo=FALSE, eval=TRUE}
frequencies_segment_8 = readRDS(url("https://github.com/U-Shift/busclar/releases/download/0.8/carris_cm_frequency_osm_052025_8h.rds"))
frequencies_segment_8 = sf::st_as_sf(frequencies_segment_8)
# tail(frequencies_segment_8)
# show this in a nice latex table
...
All code, data snapshots, and environment descriptors required to reproduce these results are archived at [U-Shift/busclar](https://github.com/U-Shift/busclar) code repository [@busclar0825].
The R and packages' versions used are listed in the `renv.lock` file.
For this article, we used `GTFShift` version v0.7.
