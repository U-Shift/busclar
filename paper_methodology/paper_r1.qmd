...
abstract: |
  ...
  The application of this methodology to the bus services in the Lisbon Metropolitan Area demonstrates a high match rate between GTFS shapes and OSM routes - above 96% for peak hour services, improving the quality of GTFS geometries and enabling more accurate spatial analysis of the public transport network.
  The release of GTFShift as an open-source R package provides a tool for researchers and practitioners to perform GTFS-OSM conflation globally, enabling a more accurate spatial analysis of the public transport network, and supporting transport planning and policy making.
...
<!--
ToDo:
- Add GTFShift version (0.10)
- Update renvlock 
- Update method for multilinestring
-->
...
An issue that has been identified in the GTFS community [@gtfs_shapes_github_discussion] and in the literature [@vuurstaek_gtfs_2020], but that remains unresolved, despite the existance of guidelines for shapes representation in GTFS [@mobilitydata_shapes_nodate], such as:

> Ensure that shapes align with the path that vehicles travel along in the WGS84 coordinate system, avoid offsets caused by differing coordinate systems.
...
Therefore, associating GTFS and OSM data requires conflation, when these tags are not filled out.
...
Some authors have tried to address the OSM-GTFS association problem.
...
Outside of the academic realm, there are several tools that connect GTFS and OSM data.
...
The workflow^[Documented at [https://u-shift.github.io/GTFShift/articles/gtfs_from_osm.html](https://u-shift.github.io/GTFShift/articles/gtfs_from_osm.html).] encompasses the following steps, further detailed:
...
Note that it is relevant to define the projected CRS to be considered for the geometrical match, to avoid distortions in the distance calculations.
For this case study, we used EPSG:3763 (PT-TM06/ETRS89) as this is the official geodetic reference system and cartographic projection for mainland Portugal.
...
1     1-TERM-FT 18957436         87.03       14.60          0 MULTILINESTRING ((-9.078279...
2     1-TERM_EA 18957437          6.77       11.87          1 MULTILINESTRING ((-9.078279...
3  1-QVB-TER_EA 18957438        281.88     2030.41          1 MULTILINESTRING ((-9.050197...
4   1-QVBB-TERM 18957439        133.49      409.43          1 MULTILINESTRING ((-9.050197...
5     1-VA-TERM 18957440         77.73       79.10          1 MULTILINESTRING ((-9.039743...
```
...
This is a crucial step, as OSM bus relations are MULTILINESTRING geometries, which consist of unordered collections of line segments (OSM ways), that if converted without a previous sorting process could lead to inconsistencies in the final GTFS shapes.
...
Once the geometry is standardized into sorted linestrings, it is converted into the GTFS `shapes` table using `gtfstools::convert_sf_to_shapes()` [@r_gtfstools]. 
...
- **Carris Metropolitana**, the urban operator for the remaining 15 municipalities in the LMA and the suburban connections among the 18.

@tbl-operator-network-characteristics presents some metrics regarding each operator's network and @fig-aml-shapes-osm depicts their spatial coverage. Carris Metropolitana is the operator with the largest network, presenting the highest values for all indicators, due to its metropolitan coverage and the suburban nature of most of its routes. The remaining operators have shorter routes and less stops per trip, as they operate mostly within their respective municipalities. Still, Carris presents a substantial number of trips, as it operates in the capital city of Lisbon, with an inherent higher demand for public transport.
...
The results, presented at @tbl-osm-match-results, demonstrate a high match rate, above 91% for all operators, that increases when considering the services at peak hour (>96%).
...
Additionally, the analysis considering the number of trips covered by the match is also presented, with a higher match rate for most operators (>96%), suggesting that the unmatched geometries are the ones with lowest frequencies. 
...
² Only displaying number of trips for peak hour because GTFS feeds from different operators cover different timespans and therefore the total number of trips is not comparable.
\end{quote}
...
MobiCascais, on the other hand, stands out for its high deviation in the start/end points distance difference, caused by the high number of route variants that are not mapped in OSM, leading to a poorer match quality.
...
TCB and Carris Metropolitana were the operators that presented the greatest reduction, with the first having dropped 9.68 percentage points in the number of shapes matched, still above 90%.
On the other hand, Carris reduced less than 3 p.p., suggesting it is the operator with the best data quality on OSM.
...
\begin{quote}
\footnotesize
\textbf{Notes:} GTFS data filtered for a working day (27/05/2026). ¹ Considering services between 08:00 and 09:00. 
\end{quote}
...
Despite not reaching a 100% match, the association with the OSM network is highly relevant in the context of spatial analysis, as it allows to relate the public transport network with an open database of the built infrastructure, providing indicators such as the number of lanes, maximum speed, type of pavement, parking existence, or the presence of bus lanes.
Analysis that rely on routing engines or catchment areas will benefit from having a common geometry, for instance, estimating travel distances or bus speeds in a more accurate and consistent way. 
...
Porto (Portugal), Madrid (Spain) or Toulouse (France) are some examples of cities that have had their public transport bus relations improved on OSM as a result of this work. 
It is worth mentioning that adding a relation in OSM does not imply changing the geometries of OSM segments.

The methods presented in this paper can be transposed beyond GTFS standard, such as NeTEx in Europe or TransXChange in the UK. Despite not supporting them natively, its open source nature allows for the extension of its functionalities to support other formats, enabling the harmonization of public transport data with OSM in different contexts and for different purposes with minimal effort.
