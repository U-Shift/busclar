...
Segments before the starting points (in gray) are discarded, as they do not meet the distance criteria, preventing geometrical inconsistencies.](figures/methodology/multiline_to_sorted_linestring_3.png){width="75%"}
...
Lisbon Metropolitan Area (LMA) was chosen as study area to demonstrate and validate the application of the GTFShift methodology presented in this paper.
With a population of 2,870,208 inhabitants [@ine_populacao_2022], it is served by a dense public transport network that covers its 18 municipalities, built up on rail, fluvial and road services provided by nine private and public operators [@tml_transportes_nodate].
...
| Operator | Shapes (#) | Shapes peak¹ (#) | Trips (#) | Trips peak¹ (#) | Shapes length² (Km) | Stops per trip² (#) | 
| :----- | ----: | ----: | ----: | ----: | ----: | ----: | 
| Carris | 263 | 176 | 10100 | 1235 | 13.16 ± 6.06 | 29.29 ± 11.71 |
| TCB | 60 | 31 | 964 | 111 | 13.71 ± 7.57 | 24.95 ± 8.99 |
| MobiCascais | 130 | 86 | 2053 | 236 | 16.23 ± 6.88 | 30.98 ± 14.84 |
| Carris Metropolitana | 1533 | 1056 | 21712 | 2900 | 22.37 ± 13.24 | 34.13 ± 16.17 |

: Operator network characteristics based on GTFS data {#tbl-operator-network-characteristics}

<small>

> **Notes:** GTFS data filtered for 27/05/2026 (working day). ¹ Considering services between 08:00 and 09:00. ² Mean value and standard deviation.
...
Additionally, the analysis considering the number of trips covered by the match is also presented, with a higher match rate, suggesting that the geometries matched are the ones with highest frequencies. 

| Operator | Matched shapes (%) | Matched shapes peak¹ (%) | Matched trips (%) | Matched trips peak¹ (%) |
| :----- | ----: | ----: | ----: | ----: | 
| Carris | 98.47 | 98.86 | 99.00 | 99.11 | 
| TCB | 91.66 |  100 | 96.78 | 100 | 
| MobiCascais | 95.45 | 97.73 | 99.10 | 99.17 |
| Carris Metropolitana | 97.39 | 98.20 | 98.59 | 98.62 |

: Results of the match between GTFS routes shapes and OSM route relations for each operator {#tbl-osm-match-results}

<small>

> **Notes:** GTFS data filtered for 27/05/2026 (working day). ¹ Considering services between 08:00 and 09:00. 
...
Carris Metropolitana consistently exhibits the highest deviations, particularly in route length (336.1 ± 1113.1 m) and start/end point discrepancy (227.4 ± 937.3 m), which is expected given its broader metropolitan coverage.
In contrast, TCB and MobiCascais demonstrate tighter spatial bounds, with MobiCascais achieving the lowest mean route length difference (110.2 ± 375.7 m), reflecting greater geometry consistency at the municipality level.

Interestingly, despite these spatial variances, the discrepancy in the number of stop points remains relatively consistent across all four operators, ranging between a mean of 24.8 and 35.2 stops.
A manual validation of several routes was performed to understand the reason behind this difference, revealing that OSM route relations tend to miss several intermediate stops, suggesting a higher effort from the community in mapping route geometries rather than the detailed stop itinerary.

| Operator | Heuristic length (m)¹ | Heuristic points (m)¹ | Heuristic stops (#)¹ |
| :---- | ---- | ---- | ---- |
| Carris | 245.2 ± 1183.5 | 76.3 ± 246.4 | 28.2 ± 11.8 |
| TCB | 157.4 ± 227.1 | 90.0 ± 360.5 | 24.8 ± 10.1 |
| MobiCascais | 110.2 ± 375.7 | 117.8 ± 269.6 | 33.0 ± 15.7 |
...
MobiCascais and Carris Metropolitana were the operators that presented the greatest reduction, with the latest having dropped 14.3 p.p.
in the number of shapes matched.
On the other hand, Carris reduced less than 4 percentage points, suggesting it is the operator with the best data quality on OSM.

| Operator | Matched shapes after filtering (#) | Matched shapes peak¹ (%) | Matched trips peak¹ (%) |
| :----- | ---- | ---- | ---- | 
| Carris | 247 | 94.89 | 95.95 |
| TCB | 52 | 90.32 | 96.39 |
| MobiCascais | 113 | 86.36 | 89.67 |
| Carris Metropolitana | 1249 | 83.90 | 82.10 |
...
When investigating which shapes do not reach a full match, it revealed that they are mostly associated with shortenings related to the first or last trips of the day that depart from or end at the dispatcher, or variants that have less frequency and are therefore less likely to be mapped in OSM.
...
The match between GTFS shapes and OSM routes had a high match rate for all operators, finding a correspondence for more than 95% of all trips at peak hour, which enables a realistic analysis of the public transport network.
